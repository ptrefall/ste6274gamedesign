...
SocketPackTask::SocketPackTask(Client &client, QTcpSocket &socket, QObject *parent)
	: QObject(parent), client(client), socket(socket)
{
}

void SocketPackTask::run()
{
	QByteArray block;
...
void SocketPackTask::packRequest(gp_header_prefix &prefix, gp_header &header, QDataStream &in)
{
...
void SocketPackTask::packAnswer(gp_header_prefix &prefix, gp_header &header, QDataStream &in)
{
